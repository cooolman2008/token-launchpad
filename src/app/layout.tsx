...
export default function RootLayout({ children }: { children: React.ReactNode }) {
	return (
		<html lang="en">
			<body className={plus_Jakarta_Sans.className + " pb-4"}>
				<nav className="flex px-8 py-5 h-20 text-gray-400">
					<Link href={"/"} className="pr-8">
						<Image id="box" src={logo} style={{ width: "auto", height: "100%" }} alt="SAFU Launcher Logo" />
					</Link>
					...
					<Link href={"/launch"} className="self-center px-2 pb-0.5 border border-white/50 rounded-2xl">
						<span className="hover:text-pink-500 text-pink-600">Launch</span>
					</Link>
