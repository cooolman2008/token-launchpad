...
const Details = memo(() => {
  ...
  useEffect(() => {
    async function fetchSafuDetails() {
      ...
      setSafuTVL(
        safuDetails?.totalLiquidityUSD ? safuDetails?.totalLiquidityUSD : 0
      );
      setSafuVolume(
        safuDetails?.totalVolumeUSD ? safuDetails?.totalVolumeUSD : 0
      );
  ...
  return (
    <>
      <div className="flex flex-col px-12">
        ...
        <h2 className="text-5xl">${getAbr(Number(safuTVL))}</h2>
      </div>
      <div className="flex flex-col px-12">
        <span className="text-lg mb-2 text-gray-400">SAFU Volume</span>
        <h2 className="text-5xl">${getAbr(Number(safuVolume))}</h2>
      </div>
      <div className="flex flex-col px-12">
        <span className="text-lg mb-2 text-gray-400">SAFU Launches</span>
        <h2 className="text-5xl">{getAbr(Number(safuLaunches))}</h2>
      </div>
    </>
  );
});
